...
import { useBinancePrices, useBinancePrice, useBinancePricesForSymbols } from '@/provider/binance';
import { useCryptoData } from './useCrypto';

// Hook pour combiner les données MongoDB avec les prix Binance
export function useCryptoWithBinancePrices() {
  const { data: mongoData, isLoading: mongoLoading, error: mongoError } = useCryptoData();
  const { prices: binancePrices, isLoading: binanceLoading, error: binanceError } = useBinancePrices();

  const combinedData = useMemo(() => {
    if (!mongoData || !binancePrices) return [];

    return mongoData.map(crypto => {
      ...
      return {
        ...
        binanceSymbol: binancePrice ? (
          binancePrices[usdtSymbol] ? usdtSymbol :
          binancePrices[btcSymbol] ? btcSymbol :
          binancePrices[ethSymbol] ? ethSymbol : binanceSymbol
        ) : null,
...
export function usePriceComparison() {
  ...
  const comparison = useMemo(() => {
    ...
    return data
      .filter(crypto => crypto.hasBinancePrice && crypto.price)
      .map(crypto => {
        const mongoPriceNum = typeof crypto.price === 'string' ? parseFloat(crypto.price) : crypto.price;
        const binancePriceNum = crypto.binancePrice!;
        
        const difference = binancePriceNum - mongoPriceNum;
        const percentageDiff = mongoPriceNum !== 0 ? (difference / mongoPriceNum) * 100 : 0;
